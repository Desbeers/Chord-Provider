...
extension JSONUtils {
    ...
    public static func decode<T: Decodable>(_ data: Data, struct: T.Type) throws -> T {
        ...
        do {
            ...
            throw ChordProviderError.jsonDecoderError(
                error: "Failed to decode due to missing key '\(key.stringValue)' not found - \(context.debugDescription)"
            )
        } catch DecodingError.typeMismatch(_, let context) {
            throw ChordProviderError.jsonDecoderError(
                error: "Failed to decode due to type mismatch - \(context.debugDescription)"
            )
        } catch DecodingError.valueNotFound(let type, let context) {
            throw ChordProviderError.jsonDecoderError(
                error: "Failed to decode due to missing \(type) value - \(context.debugDescription)"
            )
