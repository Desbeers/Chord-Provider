//
//  diagram.h
//  ChordProviderGnome
//
//  © 2025 Nick Berendsen
//

/* Limits */

#define MAX_STRINGS 6
#define MAX_BARRES  5

/* Structures */

struct cbarre {
    int finger;
    int fret;
    int start;
    int end;
};

struct cchord {
    int strings;
    int base_fret;
    int frets[MAX_STRINGS];
    int fingers[MAX_STRINGS];
    struct cbarre barres[MAX_BARRES];
    char *notes[MAX_STRINGS];
    bool show_notes;
};

typedef struct cchord cchord;
typedef struct cbarre cbarre;

/* Inits */

cchord *
cchord_new(int strings) {
    if (strings <= 0 || strings > MAX_STRINGS)
        strings = MAX_STRINGS;

    cchord *c = calloc(1, sizeof *c);
    c->strings = strings;
    c->base_fret = 1;
    return c;
}

void
cchord_free(cchord *c) {
    if (!c)
        return;

    for (int i = 0; i < MAX_STRINGS; i++) {
        free(c->notes[i]);
        c->notes[i] = NULL;
    }

    free(c);
}

/* Properties */

void
cchord_set_strings(cchord *c, int strings) {
    if (!c)
        return;

    c->strings = strings;
}

int
cchord_get_strings(const cchord *c) {
    return c ? c->strings : 0;
}

void
cchord_set_base_fret(cchord *c, int base_fret) {
    if (!c)
        return;

    c->base_fret = base_fret;
}

int
cchord_get_base_fret(const cchord *c) {
    return c ? c->base_fret : 0;
}

void
cchord_set_show_notes(cchord *c, bool show) {
    if (!c)
        return;

    c->show_notes = show;
}

bool
cchord_get_show_notes(const cchord *c) {
    return c ? c->show_notes : false;
}

/* Frets & Fingers */

void
cchord_set_fret(cchord *c, int string, int fret) {
    if (!c || string < 0 || string >= c->strings)
        return;

    c->frets[string] = fret;
}

int
cchord_get_fret(const cchord *c, int string) {
    if (!c || string < 0 || string >= c->strings)
        return 0;

    return c->frets[string];
}

void
cchord_set_finger(cchord *c, int string, int finger) {
    if (!c || string < 0 || string >= c->strings)
        return;

    c->fingers[string] = finger;
}

int
cchord_get_finger(const cchord *c, int string) {
    if (!c || string < 0 || string >= c->strings)
        return 0;

    return c->fingers[string];
}

/* Barre */

void
cchord_set_barre(
    cchord *c,
    int index,
    int finger,
    int fret,
    int start_string,
    int end_string
) {
    if (!c || index < 0 || index >= MAX_BARRES)
        return;

    c->barres[index].finger = finger;
    c->barres[index].fret   = fret;
    c->barres[index].start  = start_string;
    c->barres[index].end    = end_string;
}

void cchord_clear_barre(cchord *chord, int index);

void
cchord_clear_barre(cchord *c, int index) {
    if (!c || index < 0 || index >= MAX_BARRES)
        return;

    c->barres[index].finger = 0;
    c->barres[index].fret   = 0;
    c->barres[index].start  = 0;
    c->barres[index].end    = 0;
}

struct cbarre
cchord_get_barre(const cchord *chord, int index) {
    struct cbarre empty;
    memset(&empty, 0, sizeof(struct cbarre));

    if (!chord || index < 0 || index >= MAX_BARRES)
        return empty;

    return chord->barres[index];
}

/* Notes */

void
cchord_set_note(cchord *c, int string, const char *note) {
    if (!c || string < 0 || string >= c->strings)
        return;

    free(c->notes[string]);
    c->notes[string] = note ? strdup(note) : NULL;
}

const char *
cchord_get_note(const cchord *c, int string) {
    if (!c || string < 0 || string >= c->strings)
        return NULL;

    return c->notes[string];
}

/* Swift bridge */

extern void
draw_chord_swift(cairo_t *cr, int width, int height, gpointer user_data, gboolean dark_mode);
