...
extension Widgets {
    ...
    struct BundlePicture: AdwaitaWidget {
        ...
        func container<Data>(data: WidgetData, type: Data.Type) -> ViewStorage where Data: ViewRenderData {
            let file = g_file_new_for_path(resource)
            let storage = ViewStorage(gtk_picture_new_for_file(file)?.opaque())
